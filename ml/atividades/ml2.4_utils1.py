...
if __name__ == "__init__":
    ...
    #var1 = np.sum(np.asarray(x_test-np.mean(x_test))**2)/len(x_test) 
#var2 = np.var(x_test)
#std1 = np.sqrt(var1)
#std2 = np.std(x_test)
#var = 1 / beta + np.sum(A_train @ np.linalg.inv(beta_ml) * A_train, axis=1)
#N1 = (1/np.sqrt(2*np.pi*var1)) * np.exp( (-1/(2*var1))*((Err_test-Err_test[400])**2)  )
#N2 = norm.pdf(x_test, np.mean(x_test), np.std(x_test))
#N3 = norm.pdf(sup_x,y_test[400],Err_test[400])
#N4 = np.random.normal(y_test[400],Err_test[400],len(x_test))
#N5 = norm.pdf(y_test,np.mean(np.square(y_test-t_test)),ErmsV)
#N6 = norm.pdf(np.linspace(-1.1,1.1,1000), y_test[400], Err_test[400])
#N7 = norm.pdf(Err_test, y_test[400], Err_test[400])
