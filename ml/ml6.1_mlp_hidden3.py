...
# M1 = M2 = M3 = np.random.randint(D+1,D*2) #np.linspace(D+1,D*3,10)
# epochs = [10, 100, 1000] # n_iterações
# eta = [10**(-2), 10**(-2), 10**(-2), 10**(-4), 10**(-5)]
...
W3 = np.random.randn(M3,M2+1)
W4 = np.random.randn(K,M3+1)
...
#for epocas in epochs:
    ...
    for idx_treino, idx_teste in kfold.split(X, t):
        ...
        for n_iter in range(epocas):
            ...
            for n in range(N):
                ...
                elif h_a == 'tanh':  ha1 = np.tanh(aj1) #(np.exp(aj1) - np.exp(-aj1))/(np.exp(aj1) + np.exp(-aj1))
                zj1 = np.array(np.concatenate([np.ones(1), np.asarray(ha1)]))
                #% 2a Camada Oculta
                aj2 = W2.dot(zj1)
                if h_a == 'sigmoid': ha2 = 1./(1 + np.exp(-aj2))
                elif h_a == 'tanh':  ha2 = np.tanh(aj2) #(np.exp(aj2) - np.exp(-aj2))/(np.exp(aj2) + np.exp(-aj2))
                zj2 = np.array(np.concatenate([np.ones(1), np.asarray(ha2)]))
                #% 3a Camada Oculta
                aj3 = W3.dot(zj2)
                if h_a == 'sigmoid': ha3 = 1./(1 + np.exp(-aj3))
                elif h_a == 'tanh':  ha3 = np.tanh(aj3) #(np.exp(aj3) - np.exp(-aj3))/(np.exp(aj3) + np.exp(-aj3))
                zj3 = np.array(np.concatenate([np.ones(1), np.asarray(ha3)]))
                #% Saída
                y = W4.dot(zj3)
                ##### BACKWARD ######
                tn = tt[n]
                deltaK = y-tn # Calculo do Erro >> [erro * sigma'(ak)]
                #% Propagação do erro na 3a camada oculta
                if h_a == 'sigmoid': dz3 = zj3 * (1-zj3)
                elif h_a == 'tanh':  dz3 = 1 - zj3**2
                deltaJ3 = dz3 * (W4.T @ deltaK)
                deltaJ3 = deltaJ3[1:]
                #% Propagação do erro na 2a camada oculta
                if h_a == 'sigmoid': dz2 = zj2 * (1-zj2)
                elif h_a == 'tanh':  dz2 = 1 - zj2**2
                deltaJ2 = dz2 * (W3.T @ deltaJ3)
                deltaJ2 = deltaJ2[1:]
                ...
                deltaJ3 = deltaJ3.reshape(-1,1)
                deltaJ2 = deltaJ2.reshape(-1,1)
                deltaJ1 = deltaJ1.reshape(-1,1)     
                #% Atualiza pesos
                W4 = W4 - eta * deltaK.dot(zj3.T)
                W3 = W3 - eta * deltaJ3.dot(zj2.T)
    ...
    custo_medio = np.array(COST).mean()
    print(f'Acc Média: {acc_media}')
    
    FINAL_3h.append((h_a,3,M1,epocas,eta,acc_media,custo_medio))

df_3h = pd.DataFrame(FINAL_3h, columns=['h_a','n_hidden','M','epochs','eta','acc_mean','cost_mean'])
