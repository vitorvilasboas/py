...
class AG:
    ...
    def evaluateMoving(self, fit, localizacao):
        if localizacao in self.amb.hole: fit += self.pesos.dead
        elif localizacao in self.amb.wumpus: fit += self.pesos.dead
        elif localizacao in self.amb.validMove: fit += self.pesos.okmove
        else: fit += self.pesos.errmove
        return fit
    ...
    def objetivo(self, individuo):
        ...
        for i in range(0, len(individuo.cromossomo)):
            ...
            if gene in genesMov:
                ...
                fit = self.evaluateMoving(fit, individuo.visitou[-1]);
    ...
    def avaliaPopulacao(self, pop):
        for individuo in pop:
            individuo.fitness = self.objetivo(individuo)
