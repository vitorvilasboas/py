...
class AG:   
        ...
        if local in self.amb.hole: In.errGraves += 1
            ...
            if In.pegou and In.matou: # se já pegou e matou retornar é bom
                ...
                if local in In.visitou:
                    In.acertos += 2/(float(local[0]+1)) + 2/(float(local[1]+1)) # fora da vrf abaixo, para casos em voltar por um caminho desconhecido é mais barato
                    # In.acertos += 1
                    ...
                    if not (local in In.visitou2):
                        ...
                        In.cont = cont
                        if (In.cont < 1) or (In.cont > 2): In.errGraves += 1
                        else: In.acertos += (int)(20/(float(In.cont)**2))
                        In.visitou2.append(local)
                    
                    else: In.errGraves += 5    
                
                else: In.errGraves += 5 # Voltando por rota desconhecida, onde ainda não visitou (vagando/vicio)
    ...
    def cruzamento_corte3x(self, p1, p2, f1, f2):
        ...
        f1.cromossomo = np.concatenate((p1.cromossomo[0:rand1a], p2.cromossomo[rand2a:rand2b], 
                                        p1.cromossomo[rand1b:rand1c], 
                                        p2.cromossomo[rand2c:len(p2.cromossomo)]))
        f2.cromossomo = np.concatenate((p2.cromossomo[0:rand2a], p1.cromossomo[rand1a:rand1b], 
                                        p2.cromossomo[rand2b:rand2c],
                                        p1.cromossomo[rand1c:len(p1.cromossomo)]))
