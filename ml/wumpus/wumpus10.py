...
class AG:   
    ...
    def evaluateMoving(self, individuo):
        ...
        if localizacao in self.amb.hole: 
            ...
            for i in range(len(individuo.visitou) - 1):
                if localizacao == individuo.visitou[i]: cont += 1
            if cont > 1: individuo.retornou += 1 # se ainda nao pegou nem matou retornar é ruim  
            else: individuo.moveuok += 1   
            individuo.fitness += self.pesos.okmove/cont         
        else: 
    ...
    def objetivo(self, individuo):
        ...
        for i in range(0, len(individuo.cromossomo)):
            #print(individuo.cromossomo[0:i], individuo.fitness)
            ...
            if individuo.pegou and individuo.matou:
                ...
                print('matou e pegou')
                if individuo.visitou[-1] not in listretorno:
                    listretorno.append(individuo.visitou[-1])
                
                
                if gene in genesMov: # se já pegou e já matou retornar é bom 
                    print('voltando...')
                    individuo.fitness += 3000 * len(listretorno)
                    if gene == 'N':   mxy = (-1,0) # movimenta o individuo para o Norte (xatual+1,yatual+0)
                    elif gene == 'S': mxy = (1,0)   
                    elif gene == 'L': mxy = (0,1)
                    elif gene == 'O': mxy = (0,-1)
                    posAtual = listretorno[-1]   
                    ...
                    if posFutura in self.amb.start:
                        ...
                        individuo.fitness += 10000
                    
                    else:
                        # print(posFutura)
                        if posFutura in self.amb.hole:
                            #print(self.amb.hole)
                            individuo.fitness = self.pesos.dead
                            individuo.morreu += 1
                        elif posFutura in self.amb.wumpus:
                            individuo.fitness = self.pesos.dead
                            individuo.morreu += 1

                        # if posFutura in self.amb.seguros:
                        elif posFutura in individuo.visitou and posFutura not in listretorno:
                            # individuo.moveuok += 1
                            #print(posFutura)
                            individuo.fitness += 1000
                            listretorno.append(posFutura)
                        else:
                            if individuo.id != 'otimo':
                            # individuo.vagando += 1
                                individuo.fitness = self.pesos.errmove
