...
class Individuo:
    ...
    def avaliaMovimento(self, gene):
        ...
        if local in self.cenario.loc_buracos: self.erroFatal += 1
            ...
            if self.pegou and self.matou: # se já pegou e matou retornar é bom
                self.acertos += 20/(float(local[0]+1)) + 20/(float(local[1]+1)) # qto mais próximo da largada melhor
                if local in self.trajetoria:
                    self.acertos += 1
                    #self.acertos += 20/(float(local[0]+1)) + 20/(float(local[1]+1)) # qto mais próximo da largada melhor
                    
                    '''
                    if local in self.rotaRetorno: self.erroFatal += 5 # já voltou por ai
                    else:
                        cont = 0
                        for l in self.trajetoria: 
                            if local == l: cont += 1 #qtas vezes o local atual foi vizitado
                        if cont>2: self.erroFatal += cont # qto menos melhor
                        else:
                            self.acertos += (int)(20/(float(cont)**2))
                            self.rotaRetorno.append(local)
                    '''
                else: self.erroFatal += 5 # Voltando por rota desconhecida, onde ainda não visitou (vagando/vicio)
                if local == self.cenario.pos_largada: self.venceu = True
            else: # se ainda nao pegou nem matou retornar é ruim
                if local in self.trajetoria: self.erroFatal += 1 # Voltando por onde já passou (não explora/vicio)
    ...
    def calculaNota(self):
        ...
        if self.erroFatal > 0: self.nota = 1
            ...
            total_perdas = 10*self.erros + 3*len(self.trajetoria)
            total_ganhos = 50*self.conquistas + 2*self.acertos
