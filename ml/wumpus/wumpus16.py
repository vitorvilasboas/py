            ...
            if In.pegou and In.matou: # se já pegou e matou retornar é bom
                if local in In.visitou:
                    ...
                    if not (local in In.visitou2):
                        ...
                        In.acertos += 20/(float(local[0]+1)) + 20/(float(local[1]+1)) # fora da vrf abaixo, para casos em voltar por um caminho desconhecido é mais barato
                ...
                if local in In.visitou: In.errLeves += 5 # Voltando por onde já passou (não explora/vicio)
