...
class Individuo:
    ...
    def avaliaTiro(self, gene):
        if self.matou: self.penalidades += 10 # se já matou, pq atirar?
        else:
            if gene == '^':   mira = (-1,0)
            ...
            else: self.penalidades += 10 # errou tiro
    
    def avaliaPegar(self, gene):
        if self.pegou: self.penalidades += 10 # se já pegou, pq tentar pegar?
        else:
            if self.trajetoria[-1] == self.cenario.loc_ouro: self.pegou = True
            else: self.penalidades += 10 # não pegou
    ...
    def avaliaMovimento(self, gene):
        ...
        if local in self.cenario.loc_buracos: self.erroFatal += 1
            ...
            if self.pegou and self.matou: # se já pegou e matou retornar é bom
                ...
                else: self.penalidades += (cont**2) #3 Voltando por rota desconhecida, onde ainda não visitou (vagando/vicio)
                if local == self.cenario.pos_largada: self.venceu = True
            else: # se ainda nao pegou nem matou retornar é ruim
                if local in self.trajetoria:
                    for l in self.trajetoria: 
                        if local == l: cont += 1 #vrf qtas vezes olocal foi vizitado (qto menos passou, mais pontos ganha)
                    self.penalidades += (cont**2) # Voltando por onde já passou (não explora/vicio)
    ...
    def calculaNota(self):
        ...
        if self.erroFatal > 0:
            self.nota = 1
            #self.nota = len(self.trajetoria)/float(self.erroFatal + self.penalidades)
        else:
...
class AlgoritmoGenetico:
    def iniciaPopulacao(self):
        ...
        for individuo in self.populacao:
            ...
            individuo.avaliacao(self.cenario)
    ...
    def atualizarPopulacao(self, filhos):
        for individuo in filhos: individuo.avaliacao(self.cenario)
    ...
    def __init__(self):
        ...
        self.individuoOtimo.avaliacao(self.cenario)
        ...
        print("G0 -> %s %s" % (self.populacao[0].cromossomo, round(self.populacao[0].nota)))
        '''
        for individuo in self.populacao:
            print('---------------------------------')
        ...
        print("\n", self.cenario.mapa)
        print("Wumpus:%s ; Ouro:%s ; Buracos:%s" % (self.cenario.loc_wumpus, self.cenario.loc_ouro, self.cenario.loc_buracos))
        
        print("\nMELHOR: G%s %s %s " % (contger, self.melhorGlobal.cromossomo, round(self.melhorGlobal.nota)))
        print("%s", self.melhorGlobal.trajetoria)
        print('EFatal:%s ; Ac:%s ; Pena:%s ; Conq:%s ; (%s)' % (self.melhorGlobal.erroFatal,
                    self.melhorGlobal.acertos, self.melhorGlobal.penalidades, self.melhorGlobal.conquistas, self.melhorGlobal.nota))
        
        
        #print("Wumpus:%s ; Ouro:%s ; Buracos:%s" % (self.melhorGlobal.cenario.loc_wumpus, self.melhorGlobal.cenario.loc_ouro, self.melhorGlobal.cenario.loc_buracos))
        

        #print('OTIMO: %s %s' % (self.individuoOtimo.cromossomo, self.individuoOtimo.nota))
