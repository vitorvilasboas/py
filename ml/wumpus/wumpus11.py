...
class AG:   
    ...
    def avaliarMovimento1(self, gene, individuo):
        ...
        if local in self.amb.hole or local in self.amb.wumpus: individuo.morreu += 1
        elif local in self.amb.validMove:
            if local in individuo.visitou: individuo.passeando += 1 # se ainda nao pegou nem matou retornar é ruim  
            else: individuo.moveuok += 1   
        else: individuo.saiu += 1    
    ...
    def avaliarMovimento2(self, gene, individuo):
        ...
        if local in self.amb.start: individuo.venceu = True
            ...
            if local in self.amb.hole or local in self.amb.wumpus: individuo.morreu += 1   
                ...
                if individuo.visitou[-1] not in individuo.rotaRetorno: individuo.rotaRetorno.append(individuo.visitou[-1])
                if local in individuo.visitou and local not in individuo.rotaRetorno: 
                    individuo.voltando += 1 # se já pegou e matou retornar é bom  
                    individuo.rotaRetorno.append(local)
                else: individuo.passeando += 1                    
        return individuo
    ...
    def avaliarPegar(self, gene, individuo):
        if individuo.visitou[-1] in self.amb.gold:
            ...
            self.amb.gold = []
        else: individuo.naopegou += 1
        return individuo
    
    def calcularNota(self, In):  
        errLeve = In.passeando
        #errRegular = 0
        errGrave = 1 + In.morreu + In.saiu + In.errouTiro + In.naopegou
        acertos = In.moveuok + In.voltando
        conquistas = 0
        if In.pegou: conquistas += 1
        if In.matou: conquistas += 1
        if In.venceu: conquistas += 1
        # penalidade = 1/len(In.visitou)
        
        if errGrave > 1: fitness = 1
        else: fitness = ((acertos + (9*conquistas))**2) - errLeve
    ...
    def avaliarIndividuo(self, individuo):
        ...
        for i in range(0, len(individuo.cromossomo)):
            ...
            genesShoot = ('W','Z','D','A')
            if gene == 'P':
                if individuo.pegou: individuo.naopegou += 1   
                else: individuo = self.avaliarPegar(gene, individuo) 
            elif gene in genesShoot:
                if individuo.matou: individuo.errouTiro += 1
                else: individuo = self.avaliarTiro(gene, individuo)      
            elif gene in genesMov:
                if not individuo.pegou and not individuo.matou: individuo = self.avaliarMovimento1(gene, individuo)
                else: individuo = self.avaliarMovimento2(gene, individuo)          
