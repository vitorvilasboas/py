...
class Individuo:
    ...
    def avaliaTiro(self, gene):
        if self.matou: self.erroFatal += 5 #penalidades se já matou, pq atirar?
        else:
            if gene == '^':   mira = (-1,0)
            ...
            else: self.erroFatal += 5 #penalidades errou tiro
    ...
    def avaliaPegar(self, gene):
        if self.pegou: self.penalidades += 5 # se já pegou, pq tentar pegar?
            ...
            else: self.erroFatal += 5 #penalidades não pegou
    ...
    def avaliaMovimento(self, gene):
        ...
        if local in self.cenario.loc_buracos: self.erroFatal += 1
            ...
            if self.pegou and self.matou: # se já pegou e matou retornar é bom
                ...
                if local in self.trajetoria:
                    for l in self.trajetoria: 
                        if local == l: cont += 1 #vrf qtas vezes o local foi vizitado (qto menos passou, mais pontos ganha)
                    self.penalidades += (cont**2)
...
class AlgoritmoGenetico:
    ...
    def __init__(self):
        ...
        print("\n", self.cenario.mapa)
        print("Wumpus:%s\nOuro:%s\nBuracos:%s\n" % (self.cenario.loc_wumpus, self.cenario.loc_ouro, self.cenario.loc_buracos))
        
        print("MELHOR: G%s %s (%s) " % (contger, self.melhorGlobal.cromossomo, round(self.melhorGlobal.nota, 2)))
        print("%s", self.melhorGlobal.trajetoria)
        print('EFatal:%s ; Ac:%s ; Pena:%s ; Conq:%s ' % (self.melhorGlobal.erroFatal,
            self.melhorGlobal.acertos, self.melhorGlobal.penalidades, self.melhorGlobal.conquistas))
