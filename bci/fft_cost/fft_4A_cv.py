...
def objective(args, bci, Z, t): # class_ids, suj):
    ...
    kf = StratifiedShuffleSplit(1, test_size=0.3, random_state=42)
    cross_acc = []
...
class_ids = [2, 3]
prefix, sufix1, sufix2 = '', '', ''
ch = range(0,118)
Fs = 100
ncsp_domain = [2,4,6,8,10,18,30,42,60,78]
space = (hp.choice('ncsp', ncsp_domain), hp.quniform('clog', -6, 0, 1))
...
for suj in subjects:
    ...
    for train, test in kf.split(Z, t): 
        ...
        ncsp, clog = int(ncsp_domain[trials.argmin['ncsp']]), int(trials.argmin['clog'])
...
# print('ACC DFT:', round(R['acc_valid'].mean(), 2), round(R['dp_val'].mean(),3))
# print('COSTS DFT:', round(R['mean_cost'].mean(),2), round(R['dp_cost'].mean(),1), round(R['mean_cost_niter'].mean(),2), round(R['dp_cost_niter'].mean(),2))
