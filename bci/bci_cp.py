...
class Processor: # CPS - Central/Chain Processing System
    ...
    def process(self, single_eeg=False):
        ...
        if single_eeg:
            ...
            kf = StratifiedShuffleSplit(1, test_size=0.5, random_state=None)
        ...
        if self.crossval:
            kf = StratifiedShuffleSplit(self.nfolds, test_size=self.test_perc, random_state=None)  
            # kf = StratifiedKFold(self.nfolds, False, random_state=42)
            cross_acc, cross_kpa = [], []
    ...
    def design_clf(self):
        ...
        if self.clf_dict['model'] == 'SVM':
            self.clf = SVC(kernel=self.clf_dict['kernel']['kf'], C=10 ** (self.clf_dict['C']), probability=True)
        ...
        if self.clf_dict['model'] == 'MLP':
            self.clf = MLPClassifier(verbose=False, max_iter=10000, learning_rate_init=10 ** self.clf_dict['eta'], activation='tanh', # self.clf_dict['activ']['af'], 
                                     hidden_layer_sizes=(int(self.clf_dict['n_neurons']), int(self.clf_dict['n_hidden'])))
        ...
        if self.clf_dict['model'] == 'DTree': self.clf = DecisionTreeClassifier(criterion=self.clf_dict['crit'], random_state=0)
