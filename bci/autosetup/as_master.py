...
class BCI():
    ...
    def evaluate(self): 
        ...
        if self.crossval:
            ...
            # print(np.unique(labelsT, return_counts=True))
            # print(np.unique(labelsV, return_counts=True))
...
if __name__ == "__main__":
    ...
    for suj in subjects:
        for class_ids in classes:
            ...
            icm = BCI(data=dt, events=ev, class_ids=class_ids, fs=info['fs'], overlap=True, split='common', csp_list=None,
            ...
            ### Tunning CSP
            icm = BCI(data=dt, events=ev, class_ids=class_ids, fs=info['fs'], overlap=True, split='half_train', csp_list=csp_list,
                      csp_reg=None, crossval=False, nfolds=5, test_perc=0.5, tmin=tmin, tmax=tmax, ncomp=ncsp, 
                      f_low=fl, f_high=fh, ap={'option':'sbcsp', 'nbands':nbands}, filt_info={'design':'DFT'}, clf=clf)
    ...
    # R = pd.read_pickle("/home/vboas/cloud/devto/BCI/as_results/sbrt20/RESULTS_01.pkl")