...
def objective_func(args, lb_utils, data1, data2, events1, events2, path_eeg, class_ids=[1,2], Fs=250, filtering={'design':'DFT'},
                   crossval=True, nfolds=5, only_train=False, tmin=0, tmax=4):
    # print(args)
    fl, fh, ncsp, nbands, clf_details = args
...
if __name__ == "__main__":
    ...
    for class_ids in [[1,2],[1,3],[1,4],[2,3],[2,4],[3,4]]: # [1,2],[1,3],[1,4],[2,3],[2,4],[3,4]
        for suj in range(1, 10):
            for ex in range(1, 11):
                ...
                path_AS_trials = 'G:/Meu Drive/devs/BCI/results/trials/' + 'A0' + str(suj) + '_ex' + str(ex) + '_' + str(class_ids[0]) + 'x' + str(class_ids[1]) + '.pkl'
                ...
                try:
                    ...
                    init_vals = [{'fl':8, 'fh':30, 'ncsp':8, 'nbands':1, 'clf':{'model':'LDA'}}]
                ...
                args = (int(best['fl']), int(best['fh']), ncsp, int(best['nbands']), clf)
                ...
                # print(suj, class_ids, ex, round(acc*100,2)*(-1), args)
                ...
                # =============================================================================
                # filtering = {'design':'IIR', 'iir_order':5}
                args2 = (4, 40, 8, 9, {'model':'SVM', 'C':-4, 'kernel':{'kf':'linear'}})
    ...
    # pd.to_pickle(A, 'G:/Meu Drive/devs/BCI/results/trials/resultado.pkl') 
    # A.to_excel('G:/Meu Drive/devs/BCI/results/trials/resultado.xlsx')
    # R = pd.read_pickle('G:/Meu Drive/devs/BCI/results/trials/resultado.pkl')
    
    
    
                A.loc[len(A)] = [suj, str(class_ids[0]) + ' ' + str(class_ids[1]), ex, acc*(-1), args]
    
    pd.to_pickle(A, 'G:/Meu Drive/devs/BCI/results/trials/resultado_A0'+ str(suj) +'_LR_0.0-2.0_.pkl')
    R = pd.read_pickle('G:/Meu Drive/devs/BCI/results/trials/resultado_A0'+ str(suj) +'_LR_0.0-2.0_.pkl')
