...
bci = BCI(None, None)
def objective(args): 
    bci.f_low, bci.f_high, bci.tmin, bci.tmax, bci.ncomp = args
...
if __name__ == "__main__": 
    ...
    R = pd.DataFrame(columns=['subj', 'A', 'B', 'fl', 'fh', 'tmin', 'tmax', 'ncsp', 'acc_dft', 'acc_iir', 'cost_dft', 'cost_iir', 'kpa_dft', 'kpa_iir'])
    ...
    for suj in subjects:
        ...
        for class_ids in classes:
            ...
            bci.crossval, bci.nfolds, bci.test_perc = crossval, nfolds, test_perc
            ...
            cost_dft, cost_iir = [], []
            for i in range(10):
                bci_dft = BCI(data, events, class_ids=class_ids, fs=info['fs'], crossval=crossval, nfolds=nfolds, test_perc=test_perc, 
                ...
                bci_iir = BCI(data, events, class_ids=class_ids, fs=info['fs'], crossval=crossval, nfolds=nfolds, test_perc=test_perc, 
                              f_low=best['fl'], f_high=best['fh'], tmin=best['tmin'], tmax=best['tmax'], 
            ...
            R.loc[len(R)] = [suj, class_ids[0], class_ids[1], best['fl'], best['fh'], best['tmin'], best['tmax'], best['ncomp'], 
                             acc_dft, acc_iir, np.mean(cost_dft), np.mean(cost_iir), kappa_dft, kappa_iir]
