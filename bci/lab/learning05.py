...
def nanCleaner(epoca):
    n_epocas, n_canais, n_amostras = epoca.shape
    for i in range(n_epocas):
        for c in range(n_canais):
            bad_idx = np.isnan(epoca[i,c, :])
            epoca[i,c, bad_idx] = np.interp(bad_idx.nonzero()[0], (~bad_idx).nonzero()[0], epoca[i,c, ~bad_idx])
    return epoca
...
if __name__ == '__main__':
    ...
    while(j <= 2):
        ...
        while(i <= 4):
            ...
            print('Acc {}-{}: {}%' .format(j,i, round(acuracia * 100, 2)))
            i += 0.5
        j += 0.5  