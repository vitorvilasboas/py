...
def csp_lda(sujeito, classe, atraso, n_componentes, fl, fh, ordem):
    ...
    for i in range(4):
        ...
        janela = transpose(XF[i][:,:,W_Start:W_End], (1, 0, 2))    
        for cont in range(1, atraso + 1):
            jAtrasada = transpose(XF[i][:,:,W_Start-cont:W_End-cont], (1, 0, 2))
            janela = concatenate([janela, jAtrasada])
        XJ.append(transpose(janela, (1, 0, 2)))
    
    XT = concatenate([XJ[0],XJ[1]]) # treinamento A e B
    XV = concatenate([XJ[2],XJ[3]]) # validação A e B
