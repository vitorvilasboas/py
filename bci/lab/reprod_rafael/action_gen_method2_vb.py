...
U_ref = compute_ref(TTA, increment, valscore) # 5, 25, acc
print(U_ref)
...
for a in range(N_RUNS):
    ...
    for j in range(1):
        ...
        for i in range(0, 5):
            ...
            # k = i
            new_data_labels = np.vstack((new_data_labels, [1, int(new_data.shape[0])]))
    ...
    action_buf1 = collections.deque(maxlen=int(ABUF_LEN))
    action_buf2 = collections.deque(maxlen=int(ABUF_LEN))
