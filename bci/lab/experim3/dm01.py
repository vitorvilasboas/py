...
if __name__ == '__main__':
    ...
    # Gráfico boxplot dos resultados de acurácia 28janelas x 9 sujeitos
    ...
    # Gráfico dispersão correlação/regressão - dimensões unicas x médias_dimensões sujeito 1
    plt.scatter(medias_dim_win_suj[:,0], dim_unicas, color = 'blue', facecolors = 'none', marker = '*')
    plt.title('Sujeito 1')
    plt.xlabel('Acurácia')
    plt.ylabel('Dimensão')
    plt.plot(medias_dim_win_suj[:,0], dim_unicas)
    ...
    for s in range(len(sujeitos)):
        plt.scatter(ini_unicos, medias_ini_win_suj[:,s], color = 'blue', facecolors = 'none', marker = 'o')
        plt.plot(ini_unicos, medias_ini_win_suj[:,s])
    ...
    for i in range(len(fins_unicos)):
        ...
        for s in range(len(sujeitos)):
            plt.scatter(Windows[indice,0], AccGlobal[indice,s])
    plt.legend(fins_unicos, loc = 'upper right', ncol = 3, fontsize=7, scatterpoints=1)
    ...
    fig = plt.figure()
    rect = fig.patch # manipular background da figura
    rect.set_facecolor('mintcream') # aterar cor de fundo da figura
