...
import statsmodels.formula.api as sm # utiliza formato do R - para calculo do r2_ajustado na regressao
...
if __name__ == '__main__':
    ...
    perc_teste = 0.7
    k_folds = 10
    classifiers = ['KNN', 'SVM', 'RNA', 'Bayes', 'Tree', 'LDA'] # KNN, SVM, RNA, Bayes, Tree, LDA
    ...
    for classifier in classifiers:
        ...
        for s in range(len(sujeitos)):
            ...
            sumario_classificador[s,1] = s+1 #s,0
            for j in range(3):
                sumario_classificador[s,j+2] = round(medias_acc_suj[s,j]*100,2) #j+1
        ...
        # Exibição para conversão em tabela
        '''
        for j in range(2,5): #1,4
            print('\n')
            for s in range(len(sujeitos)):
                 print(sumario_classificador[s,j])   
        '''
        # Gerando o coeficiente de correlação entre as dimensões e as taxas de classificações médias para cada dimensão possível do sujeito 1
        # correlacao = np.corrcoef(dim_unicas, medias_dim_win_suj[:,0])
        
        # Cálculo do coeficiente de correlação entre as dimensões e respectivas taxas de classificação média
        correlacao_simples = []
        for s in range(len(sujeitos)):
            correlacao_simples.append(np.corrcoef(dim_unicas, medias_dim_win_suj[:,s])[0,1])
        correlacao_simples = np.asarray(correlacao_simples)
        # for cor in correlacao_simples: print(round(cor,2))
            
        
        ## Modelo de Regressão e calculo R2 e R2_ajustado  - dimensões e respectivas taxas de classificação média
        ...
        for s in range(len(sujeitos)):
            ...
            y_interceptacao.append(modelo_regressao.intercept_)
            inclinacao.append(modelo_regressao.coef_)
            reg_previsoes.append(modelo_regressao.predict(var_indep))
            
            r2.append(modelo_regressao.score(var_indep, var_dep)) # Calculo R2 - indicador do quanto a variavel independente explica a variavel dependente
        ...
        # for rd in r2: print(round(rd, 2))
        
        # Regresso linear Multipla 
        modelo_regressao = LinearRegression()
        reg_previsoes = []
        r2 = []
        for s in range(len(sujeitos)):
            var_indep = Windows[:,0:3]
            var_dep = AccGlobal[:,s]
            modelo_regressao.fit(var_indep, var_dep)
            reg_previsoes.append(modelo_regressao.predict(var_indep))
            r2.append(modelo_regressao.score(var_indep, var_dep))
        ...
        # plt.axisbelow(True)
        plt.title('Acurácia do classificador LDA por sujeito (MD x ME)')
        plt.xlabel('Sujeito')
        plt.ylabel('Acurácia (%)')
        ...
        # 2. Gráfico dispersão - dimensões janelas x acurácia por sujeito
        ...
        plt.figure(8, facecolor='mintcream')
        plt.subplots(figsize=(10, 6), facecolor='mintcream')
        ...
        # 6. Gráfico dispersão correlação/regressão - dimensões janelas x acurácia por sujeito
        # 2.1. Plotagem separada por sujeito distribuidos em forma de matriz
        plt.figure(9, facecolor='mintcream')
        plt.subplots(figsize=(10, 6), facecolor='mintcream')
        for s in range(len(sujeitos)):
            plt.subplot(3,3,s+1)
            var_indep = dim_unicas.reshape(-1,1)
            ...
            plt.plot(var_indep, modelo_regressao.predict(var_indep)*100, color='red', linewidth=2, linestyle='-')
            plt.xlabel('Dimensão da janela (s)')
            plt.ylabel('Acurácia (%)')
            plt.legend(loc='upper left')
        ...
        # 7. Gráfico dispersão correlação/regressão - inicios janelas x acurácia por sujeito
        # 2.1. Plotagem separada por sujeito distribuidos em forma de matriz
        plt.figure(10, facecolor='mintcream')
        plt.subplots(figsize=(10, 6), facecolor='mintcream')
        for s in range(len(sujeitos)):
            plt.subplot(3,3,s+1)
    ...
    plt.figure(11, facecolor='mintcream')
    plt.subplots(figsize=(10, 6), facecolor='mintcream')
    for c in range(len(sumarioFull)):
        plt.subplot(1,1,1)
        plt.scatter(sumarioFull[c,:,1], sumarioFull[c,:,2], color=cores_fim[c], facecolors = cores_fim[c], marker = '.')
        plt.plot(sumarioFull[c,:,1], sumarioFull[c,:,2], color=cores_fim[c], linewidth=2, label=('{}' .format(classifiers[c])))
        plt.xlabel('Sujeito')
        plt.ylabel('Mediana da Acurácia entre as janelas (%)')
        plt.legend(loc='upper left', ncol = 2, fontsize=8, title='Modelo Classificador')
