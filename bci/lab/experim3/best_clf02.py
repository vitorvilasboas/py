...
def rotulagem(eventos, ds, trueLabels): # normaliza rotulos de eventos conforme descrição oficial do dataset
    rotulos = eventos[0][:,2]
    rotulos = np.where(rotulos==1, 1023, rotulos) # Rejected trial
    rotulos = np.where(rotulos==2, 768, rotulos) # Start trial t=0
    rotulos = np.where(rotulos==3, 1072, rotulos) # Eye movements / Unknown
...
def extraiEpocas(data, eventos, classes, Fs, Tmin, Tmax, ds, suj):
    ...
    win_begin = stamps + smin # vetor que marca as amostras que iniciam cada época com IM após a dica
    win_end = stamps + smax  # vetor que marca as amostras que findam cada época com IM

    n_epocas = len(win_begin)
    n_canais = data.shape[0]
    n_amostras = smax - smin
    ...
    for lb in classes: 
        ...
        #np.save(folder + 'npy/teste/A0' + str(suj) + ds + '_' + str(lb), epocas[idx])
...
# =============================================================================
#     fid = open('/home/vboas/devto/datasets/BCICIV_2a/fdt/epocas_t2/A01T_1.fdt', 'rb')
#     dados = np.fromfile(fid, np.dtype('f'))
#     dados = dados.reshape((72, 1000, 22)) #Dá uma nova forma a uma matriz sem alterar seus dados
#     dados = np.transpose(dados, (0,2,1))
# =============================================================================

    
    
    # Parâmetros gerais
    sujeitos = np.arange(2,3)
    classes = np.arange(1,5)
    datasets = ['T','E']
    folder = "/home/vboas/devto/datasets/BCICIV_2a/"
    Fs = 250
    janela_min = 1
    Tmin, Tmax = 0, 7.5 # Start trial= 0s ; Start dica= 2s ; End MI= 6s ; # Inicio e Fim da IM respectivamente (em segundos)
    ...
    cont = 0
    sumarioFull = []
    classifiers = ['LDA'] #['KNN', 'SVM', 'RNA', 'Bayes', 'Tree', 'LDA']
    for classifier in classifiers:
        ...
        for suj in sujeitos:
            ...
            for ds in datasets: 
                ...
                ### Rotulando corretamente os eventos conforme descrição da competição
                eventos = rotulagem(eventos,ds, trueLabels)
                
                # Extração de todas as 288 épocas (72 por classe)
                epocas = extraiEpocas(dados, eventos, classes, Fs, Tmin, Tmax, ds, suj) # [4x72x22x1000]
                
                # normalização/correção de valores NaN nas épocas
                epocas = nanCleaner(epocas)
                
                #epocas = [ np.load(folder + 'npy/teste/A0' + str(suj) + ds + '_' + str(lb) + '.npy') for lb in classes ]
