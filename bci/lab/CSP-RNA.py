...
def csp_lda(sujeito, classe, atraso, n_componentes, fl, fh, ordem):
    ...
    for i in range(2):
        for j in range(2):
            path = '/mnt/dados/bci_tools/dset42a/npy/epocas_t2/A0' + str(sujeito) + ds[i] + str(classe[j]) + '.npy'
    ...
    X0 = XT[classes[0] == y,:,:] # epocas classe A
    X1 = XT[classes[1] == y,:,:] # epocas classe B
...
if __name__ == "__main__":
    ...
    fh = 30
    ordem = 5
    result = [csp_lda(suj, classes[i], atraso, n_componetes, fl, fh, ordem) for suj in range(1, 2) for i in range(0, 1) ]
