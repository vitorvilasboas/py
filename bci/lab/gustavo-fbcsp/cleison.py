...
def make_basis(q, m, fi, fm):
    ...
    for i in range(m):
        ...
        #~ Colocamos os dois nas posições 2*i e 2*i+1 da base, de forma que ela fique organizada da seguinte forma [Seno_0, Cosseno_0, Seno_1, Cosseno_1, etc]
        X0[:,2*i] = b_sin
        X0[:,2*i+1] = b_cos
...
def get_step(n_bins, n_bands, overlap):
    ...
    #~ Porém as vezes o resultado retornado faz com que a última banda tenha bins não existentes. Para corrigir isso usei um loop que verifica o último bin da última banda e se ele não for menor ou igual ao número de bins o step é reduzidor por 2. Provavelmente existe alguma maneira de achar isso de forma não-iterativa mas como o tempo em que isso é feito é desprezível não procurei otimizar essa parte
    while True:
...
def sbcsp(subject, classes, args):
    ...
    #~ Todos os dados são janelados entre 2.5 e 4.5 segundos. Fornecendo também o tempo inicial nesses arquivos que é igual a 2 segundos e a frequência de amostragem, 250 Hz
    X = windowing(X, 2, 2.5, 4.5, 250)

    #~ Aqui eu junto os dados de treinamento e validação cada um em um tensor (XT e XV respectivamente). Cada um tem as dimensões (ne, nc, nt), e estão organizados de forma que as épocas correspondentes à primeira classe venham primeiro e depois as da segunda classe
    #~ ne = Número de épocas (144 no total, 72 para cada classe)
    #~ nc = Número de canais (22)
    #~ nt = Número de amostras no tempo (500, 2 segundos em 250 Hz)
    XT = concatenate(X[0])
    ...
    #~ Definindo um vetor de tamanho 144, onde a primeira metade são todos 0 e a segunda metade são todos 1. Isso vai ser usado como vetor de classes (gabarito)
    y = concatenate([zeros(72), ones(72)])

    #~ Aqui é feita a projeção dos sinais na base de senos e cossenos. São fornecidos as frequências limite (fl, fh) e o número de senos/cossenos na base (m). Os tensores na saída são da forma (ne, nc, 2*m) e os coeficientes de seno/cosseno ficam intercalados de forma que o vetor fique ordenado de acordo com a frequencia. Detalhes de como isso é feito estão na função filtering
    ...
    #~ step = Intervalo de coeficientes entre duas bandas, resultado do número de bins, bandas e do overlap definido. Mais detalhes na função get_step
    step = get_step(n_bins, n_bands, overlap)
    
    #~ size = Número de coeficientes em cada banda
    size = step * overlap

    #~ Aqui as bandas são separadas. Cada banda indexada por i é um pedaço do tensor original de dimensão (ne, nc, 2*m). Após isso são obtidos n_bands bandas, cada banda sendo representada por um tensor de dimensão (ne, nc, size)
    XT = [XT[:,:,i*step:i*step+size] for i in range(n_bands)]
    XV = [XV[:,:,i*step:i*step+size] for i in range(n_bands)]
    

    ## Vários (n_bands) CSPs são definidos e treinados com os dados do conjunto de treinamento (XT). XT[i] é o tensor pra banda i, e o vetor y de classes é o mesmo pra todos
    csp = [CSP(n_components=n_components) for i in range(n_bands)]
    for i in range(n_bands):
        csp[i].fit(XT[i], y)

    #~ Depois do treinamento os dados são transformados, cada XT[i] e XV[i] vira uma matriz de forma (ne, n_components). Mais detalhes do treinamento e transformação estão na classe CSP
    ...
    for i in range(n_bands):
        ...
        #~ Para cada banda, esse LDA treinado transforma os dados de treinamento e validação. Relembrando, não é feita uma classificação 'dura', o resultado contínuo do LDA é mantido para construir um vetor de características de dimensão (n_bands)
        SCORE_T[:,i] = ravel(clf[i].transform(XT_CSP[i]))
        SCORE_V[:,i] = ravel(clf[i].transform(XV_CSP[i]))


    ## Aqui é treinado o meta-classificador bayesiano. Os dados de treinamento são separados entre os pertencentes à classe 0 e classe 1. Para cada classe, calculamos a média e o desvio padrão entre as épocas, de forma que os vetores sejam de dimensão n_bands
    ...
    #~ Os scores são aplicados na função de densidade de probabilidade de cada classe e calculamos a razão entre elas. Um resultado positivo de META_SCORE_T indica que uma época é melhor representada pela função p0, já que p0.pdf(SCORE_T) > p1.pdf(SCORE_T). No caso contrário META_SCORE_T é negativo
    ...
    #~ ans == y retorna 1 quando a classe estimada é correta (igual a y) e 0 quando é incorreta. Então a média desse vetor é igual a taxa de acertos do classificador
    return mean(ans == y)
