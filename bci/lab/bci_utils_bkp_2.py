...
class BCI():
    ...
    def __init__(self, data=None, events=None, class_ids=[1,2], fs=250, overlap=True, crossval=False, nfolds=10, test_perc=0.5, 
                 f_low=None, f_high=None, tmin=None, tmax=None, ncomp=None, nbands=None, ap=None, filt_info=None, clf=None, fase=1):
        self.data = data
        self.events = events
    ...
    def evaluate(self):
        ...
        if self.crossval:
            ...
            # XT = np.concatenate([XT[0],XT[1]])
            # XV = np.concatenate([XV[0],XV[1]])
            # # XV = np.copy(XT) # Test data classes A + B
                        
            # #### Para validação da validação no auto-setup (XT=35%, XV=35%, XV2=30%)
            if self.fase==2: XV = np.r_[XT[0][:22], XV[0][:22], XT[1][:22], XV[1][:22]] # uncomment/comment
            else: XV = np.r_[XV[0][22:], XV[1][22:]] # comment/uncomment
            XT = np.r_[XT[0][22:], XT[1][22:]]
            
            
            # idx44 = np.array([1,6,12,14,16,21,31,33,39,43,45,47,52,55,61,62,64,68,70,72,74,76,77,78,79,85,88,89,
            #                   90,95,98,101,103,105,109,113,118,123,126,131,136,137,140,143])
            # idx51 = np.array([0,2,3,4,5,7,8,11,15,18,19,20,24,27,28,32,35,42,46,49,51,57,63,69,71,81,86,87,92,94,
            #                    96,104,106,108,110,111,114,115,116,117,119,122,124,125,129,130,132,135,139,142])
            # idx52 = np.array([9,10,13,17,22,23,25,26,29,30,34,36,37,38,40,41,44,48,50,53,54,56,58,59,60,65,66,67,
            #                    73,75,80,82,83,84,91,93,97,99,100,102,107,112,120,121,127,128,133,134,138,141])
            # XA = np.r_[XT[0], XV[0]]
            # XB = np.r_[XT[1], XV[1]]
            # XT = np.r_[XA[idx51],XB[idx51]]
            # # XV = np.r_[XA[idx52],XB[idx52]]
            # XV = np.r_[XA[idx44],XB[idx44]]
