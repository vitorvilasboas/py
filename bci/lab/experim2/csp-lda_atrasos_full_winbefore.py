...
if __name__ == "__main__":
    ...
    acuracias = zeros([len(sujeitos),len(atrasos),len(pares_classes)])
    tempos = zeros([len(sujeitos),len(atrasos),len(pares_classes)])
    
    acc_medias_suj = zeros([len(sujeitos),len(atrasos)])
    tempos_medios_suj = zeros([len(sujeitos),len(atrasos)])
    acc_dvpadrao_suj = zeros([len(sujeitos),len(atrasos)])
    tempos_dvpadrao_suj = zeros([len(sujeitos),len(atrasos)])
    
    acc_media_atrasos = zeros([len(atrasos)])
    tempo_medio_atrasos = zeros([len(atrasos)])
    acc_dvpadrao_atrasos = zeros([len(atrasos)])
    tempo_dvpadrao_atrasos = zeros([len(atrasos)])
    ...
    for atraso in atrasos:
        for sujeito in sujeitos:
            ...
            for classes,cl in zip(pares_classes,range(len(pares_classes))):
                ...
                acuracias[sujeito-1,atraso,cl] = acuracia
                tempos[sujeito-1,atraso,cl] = tempo
                
            acc_medias_suj[sujeito-1,atraso]        = round(mean(acuracias[sujeito-1,atraso,:])*100,1) # em %
            tempos_medios_suj[sujeito-1,atraso]     = int(mean(tempos[sujeito-1,atraso,:])*1000) # em ms (milisegundos)
            acc_dvpadrao_suj[sujeito-1,atraso]      = round(std(acuracias[sujeito-1,atraso,:])*100,1) # em %
            tempos_dvpadrao_suj[sujeito-1,atraso]   = int(std(tempos[sujeito-1,atraso,:])*1000) # em ms (milisegundos)
            
        acc_media_atrasos[atraso]       = round(mean(acuracias[:,atraso,:])*100,1) # em %
        tempo_medio_atrasos[atraso]     = int(mean(tempos[:,atraso,:])*1000) # em ms (milisegundos)
        acc_dvpadrao_atrasos[atraso]    = round(std(acuracias[:,atraso,:])*100,1) # em %
        tempo_dvpadrao_atrasos[atraso]  = int(std(tempos[:,atraso,:])*1000) # em ms (milisegundos)