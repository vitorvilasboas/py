...
if __name__ == "__main__":
    ...
    XVF = []
    if filtro == 'IIR':
        for i in range(len(XT)): # filtra os dados de treinamento e validação das duas classes 
            XTF.append( lfilter(b, a, XT[i]) )
            XVF.append( lfilter(b, a, XV[i]) )
    ...
    for i in range(len(XTF)):
        janelaT = XTF[i][:,:,W_Start:W_End]
        janelaV = XVF[i][:,:,W_Start:W_End]
        for cont in range(1, atraso + 1):
            jAtrasadaT = XTF[i][:,:,W_Start-cont:W_End-cont]
            janelaT = concatenate([janelaT, jAtrasadaT], axis=1)
            
            jAtrasadaV = XVF[i][:,:,W_Start-cont:W_End-cont]
            janelaV = concatenate([janelaV, jAtrasadaV], axis=1)
        XTJ.append(janelaT)
        XVJ.append(janelaV)
    
    # Concatenating training and evaluating data
    XTJ = concatenate([XTJ[0],XTJ[1]]) # Dados de treinamento das classes A, B
    XVJ = concatenate([XVJ[0],XVJ[1]]) # Dados de validação das classes A, B
    y = concatenate([zeros(72), ones(72)]) # Criando vetor gabarito
    
    # CSP
    csp = CSP(n_components=n_comp)
    csp.fit(XTJ, y)
    XT_CSP = csp.transform(XTJ)
    XV_CSP = csp.transform(XVJ) 
